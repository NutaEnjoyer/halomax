...
// Накопители для дельт (глобальные переменные в начале файла)
var currentUserText = "";
...
function onCallConnected() {
    ...
    OpenAI.createRealtimeAPIClient({
        ...
        // Дельты пользователя
        client.addEventListener(OpenAI.RealtimeAPIEvents.ConversationItemInputAudioTranscriptionDelta, function(event) {
            try {
                Logger.write("[DEBUG] User delta event: " + JSON.stringify(event));

                var delta = event && event.delta ? event.delta : (typeof event === "string" ? event : null);
                if (delta && typeof delta === "string") {
                    currentUserText += delta;
                    Logger.write("[DELTA USER] " + delta);
                }
            } catch (err) {
                Logger.write("[ERROR] User delta processing: " + err);
            }
        });

        // Дельты ассистента
        client.addEventListener(OpenAI.RealtimeAPIEvents.ResponseAudioTranscriptDelta, function(event) {
            try {
                Logger.write("[DEBUG] Assistant delta event: " + JSON.stringify(event));
                ...
                Logger.write("[ERROR] Assistant delta processing: " + err);
            }
        });

        // Завершение реплики пользователя
        client.addEventListener(OpenAI.RealtimeAPIEvents.ConversationItemInputAudioTranscriptionCompleted, function(event) {
            try {
                Logger.write("[DEBUG] User completed event: " + JSON.stringify(event));
                ...
                if (currentUserText.trim()) {
                    ...
                    currentUserText = "";  // сброс
                } else {
                    Logger.write("[USER COMPLETE] No text accumulated");
                }
            } catch (err) {
                Logger.write("[ERROR] User completion processing: " + err);
            }
        });

        // Завершение ответа AI (самое важное!)
        client.addEventListener(OpenAI.RealtimeAPIEvents.ResponseAudioTranscriptDone, function(event) {
            try {
                Logger.write("[DEBUG] Assistant done event: " + JSON.stringify(event));

                if (currentAssistantText.trim()) {
                    addToTranscript("assistant", currentAssistantText);
                    Logger.write("[ASSISTANT DONE] Added to transcript: " + currentAssistantText);
                    currentAssistantText = "";  // сброс
                } else {
                    Logger.write("[ASSISTANT DONE] No text accumulated");
                }
            } catch (err) {
                Logger.write("[ERROR] Assistant completion processing: " + err);
            }
        });

        // Прерывание — резко замолчать
        client.addEventListener(OpenAI.RealtimeAPIEvents.InputAudioBufferSpeechStarted, function() {
            try {
                ...
                Logger.write("[ERROR] Interruption handling: " + err);
            }
        });
